...
class ApFetchService implements FetchServiceInterface
{
    private StorageService $storageService;
    private array $config;

    /**
     * Create a new AP fetch service instance.
     *
    ...
    /**
     * Fetch resource list from AP API.
     *
    ...
    public function fetchResourceList(): array
    {
        ...
        if ('' === $apiUrl || '' === $apiKey) {
            Log::error('[ApFetchService] API 配置不完整', [
                'api_url' => $apiUrl,
                'has_api_key' => '' !== $apiKey,
            ]);
            return [];
        ...
        try {
            // TODO: Implement actual AP API call
            ...
            if (!$response->successful()) {
                Log::error('[ApFetchService] API 請求失敗', [
            ...
            // TODO: Parse AP API response format
            // Expected format will be determined when API details are available
            ...
            Log::info('[ApFetchService] 取得資源列表', [
                'count' => count($resources),
            ]);

            return $resources;
        } catch (\Exception $e) {
            Log::error('[ApFetchService] 取得資源列表失敗', [
                'error' => $e->getMessage(),
            ]);
            return [];
        }
    ...
    /**
     * Download single resource from AP API.
     *
    ...
    public function downloadResource(string $resourceId): ?array
    {
        ...
        try {
            // TODO: Implement actual AP API download call
            $response = Http::withHeaders([
                'Authorization' => 'Bearer ' . $apiKey,
            ])->get($apiUrl . '/resources/' . $resourceId);

            if (!$response->successful()) {
                Log::error('[ApFetchService] 下載資源失敗', [
    ...
    public function saveToStorage(array $resources, string $storageType): bool
    {
        ...
        $basePath = $this->config['s3_path'] ?? 'ap/';
        $successCount = 0;
        ...
        foreach ($resources as $resource) {
            try {
                ...
                if (isset($resource['xml_content'])) {
                    ...
                    Log::info('[ApFetchService] 儲存 XML', ['path' => $xmlPath]);
                }
                ...
                if (isset($resource['video_url'])) {
                    ...
                    // $videoContent = Http::get($resource['video_url'])->body();
                    // $disk->put($videoPath, $videoContent);
                    Log::info('[ApFetchService] 影片下載待實作', ['url' => $resource['video_url']]);
                }

                $successCount++;
            } catch (\Exception $e) {
                Log::error('[ApFetchService] 儲存資源失敗', [
        ...
        Log::info('[ApFetchService] 儲存資源完成', [
